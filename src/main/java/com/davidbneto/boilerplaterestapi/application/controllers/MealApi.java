...
public interface MealApi {
  ...
  @Operation(summary = "Operation performed to retrieve a Meal by uuid",
      responses = {
          @ApiResponse(
  ...
  @Operation(summary = "Operation performed to retrieve all meals",
      responses = {
          @ApiResponse(
              responseCode = HttpResponse.STATUS_OK,
              description = HttpResponse.OK,
              content = @Content(
                  mediaType = MediaType.APPLICATION_JSON_VALUE,
                  array = @ArraySchema(schema = @Schema(implementation = Meal.class))
              )),
          @ApiResponse(
              responseCode = HttpResponse.STATUS_UNAUTHORIZED,
              description = HttpResponse.UNAUTHORIZED,
